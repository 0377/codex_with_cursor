---
name: codex-with-cursor-planning
description: Plan codex-with-cursor workflows by turning a subagent or delegation request into scoped tasks, roles, acceptance criteria, and review gates before dispatch.
---

# Codex with Cursor Planning

Read `../codex-with-cursor/CODEX_WITH_CURSOR.md` before planning. Use this skill when a request needs child-agent, subagent, delegation, 子代理, 委派, or 派工 routing through codex-with-cursor.
...
- Use `../codex-with-cursor/contract.json` values for roles, status tokens, and report headings; do not invent local variants.
...
Do not run `agent` or `delegate_to_cursor.*` from the main thread.
