...
def update_session_state(
    ...
    lock = acquire_file_lock(
        ...
        timeout_seconds,
        0.1,
        lambda: f"Timed out waiting for Cursor session pool lock: {lock_path}",
    )
    try:
...
def acquire_session_lease(
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise DelegateError(
                f"Cursor primary session is leased by another delegate. SessionKey: {key}. Use a longer -SessionLeaseWaitSeconds or choose ParallelPool."
            )
        time.sleep(0.25)
...
def reset_session_lease_for_fresh_session(
    ...
    if lease is None or not lease.leased:
        raise DelegateError("Cannot reset a Cursor session lease that is not currently leased.")
    ...
    def updater(state: dict[str, Any]) -> SessionLease:
        ...
        if lease.mode in ("PrimaryReuse", "PrimaryAnchor"):
            ...
            if str(primary.get("leaseRunId")) != run_id:
                raise DelegateError(
                    f"Cannot reset primary Cursor session lease; expected run '{run_id}' but found '{primary.get('leaseRunId')}'."
                )
        ...
        raise DelegateError(f"Cannot reset parallel Cursor session lease for run '{run_id}'; the leased session was not found.")
