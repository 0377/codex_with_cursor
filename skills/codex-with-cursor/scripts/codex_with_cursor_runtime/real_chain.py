...
def run_real_chain_validation(ns: argparse.Namespace) -> int:
    root = repo_root()
    validation_root = Path(ns.validation_root).resolve() if ns.validation_root else (root / ".codex" / "codex_with_cursor" / "cursor-delegate-validation").resolve()
    ...
    ext = script_ext()
    delegate_entry = f"{rel}/{scripts}/delegate_to_cursor{ext}"
    ...
    task_specs = [
        ...
        (
            ...
            [verify_entry, chain_entry, ".codex/codex_with_cursor/cursor-delegate"],
        ...
        (
            ...
            "ParallelPool",
            "-SessionMode ParallelPool -AllowParallel",
            [delegate_entry, ".codex/codex_with_cursor/cursor-delegate"],
        ...
        (
            ...
            [delegate_entry, verify_entry, chain_entry, f"{rel}/CODEX_WITH_CURSOR.md"],
...
- SessionMode: {mode}
- Child-thread only: This task must run inside a Codex spawn_agent child thread with model 'gpt-5.4', reasoning_effort 'medium', fork_context 'false'.
- Required child-thread marker: set process environment CODEX_CURSOR_CHILD_THREAD=1 before invoking the worker entry script.
...
- Every Cursor Agent worker must run inside a child thread with:
  ...
  - fork_context: false
- Every child thread must set CODEX_CURSOR_CHILD_THREAD=1 and then call {delegate_entry} with -TaskFile.
- Do not run Cursor Agent CLI or delegate_to_cursor directly from the main thread.
