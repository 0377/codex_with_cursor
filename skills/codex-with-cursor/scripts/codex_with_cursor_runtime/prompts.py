...
def build_prompt(
    ...
    primary_entry = f"{rel}/{script_family()}/delegate_to_cursor{script_ext()}"
    windows_entry = f"{rel}/windows_scripts/delegate_to_cursor.ps1"
    macos_entry = f"{rel}/macos_scripts/delegate_to_cursor.sh"
...
You are Cursor Agent acting as an implementation worker for Codex.
...
- Never inspect, poll, or wait on the current run's own live artifacts (`status_{run_id}.json`, `stream_{run_id}.jsonl`, `trace_{run_id}.log`, `config_{run_id}.json`, `prompt_{run_id}.md`, `cursor_{run_id}.md`) as task input. Those files belong to the wrapper for this run, not to the delegated task.
...
Hard requirements:
- Read {rel}/CODEX_WITH_CURSOR.md before scanning other repository files.
- Use {rel}/CODEX_WITH_CURSOR.md as the single workflow contract for delegation, audit flow, session mode interpretation, and worker report requirements.
