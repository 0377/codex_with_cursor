...
from .cursor_cli import retry_decision
...
def run_delegate_subprocess(args: list[str], env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, str(runtime_python_root() / "delegate_to_cursor.py"), *args],
        cwd=str(repo_root()),
        text=True,
        capture_output=True,
        env=merged_env,
    ...
    with tempfile.TemporaryDirectory(prefix="codex_with_cc_delegate_runtime_") as tmp:
        ...
        write_text(
            ...
            (
                ...
                "from codex_with_cursor_runtime.process_cleanup import install_child_process_cleanup\n"
        ...
        output_text = read_text(next(run_root.glob("cursor_*.md")))
        ...
        retry_output = read_text(next(retry_root.glob("cursor_*.md")))
        ...
        output_path = verify_root / f"cursor_{run_id}.md"
