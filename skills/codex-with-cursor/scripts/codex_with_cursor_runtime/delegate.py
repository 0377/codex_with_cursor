...
Summary
The delegate run did not reach Cursor Agent execution.
...
Findings
- Delegate worker failed before Cursor Agent execution started.
...
Summary
Delegate dry run completed without executing Cursor Agent.
...
Findings
- Dry run enabled; Cursor Agent was not invoked.
...
def build_artifact_paths(ns: argparse.Namespace, root: Path, workflow_id: str) -> DelegateArtifactPaths:
    ...
    output_path = Path(ns.output_path).resolve() if ns.output_path else (artifact_root / f"cursor_{run_id}.md").resolve()
...
def complete_dry_run(paths: DelegateArtifactPaths, status: dict[str, Any], config: dict[str, Any], lease: SessionLease, role: str) -> None:
    ...
    write_text(paths.trace_path, f"[dry-run] Cursor Agent was not invoked for run {paths.run_id}\n")
...
def run_delegate(ns: argparse.Namespace) -> int:
    if os.environ.get(CHILD_MARKER_NAME) != CHILD_MARKER_VALUE:
        raise DelegateError(
            f"delegate_to_cursor may only run inside a Codex spawn_agent child thread. Missing required child-thread marker '{CHILD_MARKER_NAME}={CHILD_MARKER_VALUE}'. Main-thread/direct invocation is forbidden."
        )
    ...
    entry_path = workflow_root() / "CODEX_WITH_CURSOR.md"
    ...
    try:
        if not ns.allow_parallel:
            def lock_message() -> str:
                ...
                return f"Another delegate_to_cursor run is still active. Use -AllowParallel to bypass, or wait. Lock: {lock_path}. Holder: {holder}"
    ...
    try:
        ...
        print(f"Delegating to Cursor Agent: {resolve_agent_executable() or '<dry-run>'}")
        print(f"RunId: {run_id}")
        print(f"Session Name: {effective_name}")
        print(f"Session Mode: {ns.session_mode}")
        print(f"Session Key: {key}")
        print(f"Artifact Root: {artifact_root}")
        print(f"Cursor Session Id: {lease.session_id}")
        print(f"Cursor Session Argument: {'--resume' if lease.resume else '<new session>'} {lease.session_id}")
        ...
        if ns.dry_run:
            print("Dry run enabled; Cursor Agent was not invoked.")
        ...
        with raw_stream_path.open("w", encoding="utf-8") as raw_handle, trace_path.open("w", encoding="utf-8") as trace_handle:
            while attempt < max_attempts:
                ...
                cursor_args = new_cursor_cli_args(
                    ns.model,
                    lease.session_id,
                    lease.resume,
                ...
                if observed_session and str(observed_session) != lease.session_id:
                    lease = dataclasses.replace(lease, session_id=str(observed_session))
                    config["sessionId"] = lease.session_id
                    write_json(config_path, config)
                ...
                if decision["shouldRetry"] and attempt < max_attempts:
                    ...
                    if decision["retryWithFreshSession"]:
                        print(f"WARNING: Cursor Agent rejected resumed session '{lease.session_id}'. Retrying once with a fresh session.", file=sys.stderr)
                        trace_handle.write("[retry] stale session id rejected; resetting lease for a fresh Cursor session\n")
                        trace_handle.flush()
                        ...
                        print("WARNING: Cursor Agent completed without the required report headings. Retrying once for structured report repair.", file=sys.stderr)
                        trace_handle.write("[retry] unstructured success; asking the same Cursor session to emit the required report headings\n")
                        trace_handle.flush()
                        prompt_text = build_report_repair_prompt(output_path, final_text)
                        lease = dataclasses.replace(lease, resume=True)
                    else:
                        print("WARNING: Cursor Agent startup failed before structured output was produced. Retrying once with the current session arguments.", file=sys.stderr)
                ...
                if decision["shouldRetry"]:
                    ...
                    exit_code = 1
                    attempt_record["cursorExitCode"] = cursor_exit_code
...
Findings
- Delegate worker detected a retryable Cursor Agent failure and exhausted the configured retry budget.
...
Risks Or Follow-ups
- Inspect Cursor Agent CLI startup/session health before retrying this delegated task again.
"""
                ...
                if output_resolution["outputWasNormalized"]:
                    ...
                    attempt_record["cursorExitCode"] = exit_code
                    exit_code = 1
                    attempt_record["exitCode"] = exit_code
                    failure_disposition = "NEED_HUMAN_INTERVENTION"
                    failure_summary_text = (
                        "UNSTRUCTURED_SUCCESS_REJECTED: Cursor Agent exited with code 0 but did not produce the required "
        ...
        if output_resolution["shouldPersistFinalText"]:
            ...
            write_text(output_path, "Cursor delegate finished without a structured text result.")
        ...
        if exit_code != 0:
            if failure_disposition == "NEED_HUMAN_INTERVENTION":
                raise DelegateError(f"Cursor delegate retry ceiling reached: {failure_summary_text}")
            raise DelegateError(f"Cursor Agent exited with code {exit_code}")
        if status["status"] != "completed":
            if failure_disposition == "NEED_HUMAN_INTERVENTION":
                raise DelegateError(f"Cursor delegate retry ceiling reached: {failure_summary_text}")
            raise DelegateError(f"Cursor Agent finished without the required structured delegate report headings. Output: {output_path}")
        return 0
    except Exception as exc:
