...
def assert_no_legacy_delegate_flags(argv: list[str] | None) -> None:
    for arg in argv or []:
        normalized = str(arg).split("=", 1)[0]
        if normalized in ("-Task", "-Mode"):
            raise DelegateError(
                f"Legacy delegate argument {normalized} is forbidden. "
                "Use -TaskFile instead of inline -Task, and -Role instead of -Mode."
            )


def main(argv: list[str] | None = None) -> int:
    assert_no_legacy_delegate_flags(argv)
    parser = build_parser()
