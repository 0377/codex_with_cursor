...
def verify_artifacts(run_id: str, artifact_root_value: str | None) -> dict[str, Any]:
    ...
    output_path = Path(str(config.get("outputPath") or (root / f"cursor_{run_id}.md"))).resolve()
...
def run_verify_chain(ns: argparse.Namespace) -> int:
    ...
    for record in reuses:
        ...
        if str(final.get("sessionId")) != expected_main_session_id:
            ...
            if record["status"].get("lastRetryReason") != "stale_cursor_session":
                raise DelegateError(f"Reuse run '{record['runId']}' must record stale_cursor_session when changing primary session.")
    ...
    if stale_reset_occurred:
        ...
        if primary.get("lastResetReason") != "stale_cursor_session":
            raise DelegateError("Primary session reset reason must be stale_cursor_session.")
