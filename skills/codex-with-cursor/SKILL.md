...
This skill is distributed through a plugin-managed installation from [this repository](https://github.com/0377/codex_with_cursor) (`codex plugin marketplace add` then `codex plugin add`; update with `codex plugin marketplace upgrade`). When invoking bundled scripts, run them from the target project's current working directory so `.codex/codex_with_cursor` tasks and artifacts are written to that project, not to the plugin cache.
...
4. Dispatch gate: use fresh child threads with `model: gpt-5.4`, `reasoning_effort: medium`, and `fork_context: false`.
