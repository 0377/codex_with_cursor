...
现在你 PUA Codex，让 Codex 再 PUA Cursor Agent，先让 AI 自己内卷一波！你再做Codex验收结果的验收！

以前你让 Codex 一个人硬扛大项目，它要读代码、拆需求、找调用链、改文件、跑测试、看失败日志、再回头修。主线程越塞越胖，token 越烧越猛，这就好比你花重金请了马斯克，结果让他去通下水道？！暴殄天物啊！

现在？**欢迎来到 AI 黑心包工队！**

Codex 稳坐老板椅发号施令，子代理排队领活干，Cursor Agent 披着 DeepSeek 的皮疯狂下场搬砖。什么长上下文探索？什么大范围重构？什么无限死循环排错？不再往主线程里硬灌，直接扔给执行层干，AI包工队不配有双休！
...
1. 安装 Cursor Agent。
2. 安装 CC Switch。
3. 在 CC Switch 里把 Cursor Agent 的后端 API 切到 DeepSeek。
...
它做的事情更朴素，也更工程化：把一套 `Codex -> Codex 子代理 -> Cursor Agent CLI` 的委派工作流复制进任意项目，让 Codex 当 leader，Cursor Agent/DeepSeek 当执行层。脏活累活、长上下文探索、大范围改代码、互相找茬，都扔给子代理；主 Codex 只管拆解、调度、验收、打回重改。
...
```text
Codex 主线程 -> Codex 子代理 -> Cursor Agent CLI -> Cursor Agent 后端模型
```
...
Codex 子代理负责作为可追踪的对话树节点，调用委派脚本，把具体实现、调查、审查这些高 token 消耗任务交给 Cursor Agent CLI。它不是随便开的聊天窗口，而是主线程派出去的任务节点。

Cursor Agent CLI 负责执行被委派的具体任务。它可以做调查、改文件、运行验证，并输出结构化报告。

如果 Cursor Agent 后端通过 CC Switch 接到 DeepSeek，那么大量读代码、改代码、跑验证的苦活就可以交给 DeepSeek 去消化。README 里提到的重点体验，是 DeepSeek 的缓存命中率很夸张，重复阅读、重复建模、重复烧 token 的部分能少一点是一点。

所以这个链路的本质不是“让多个模型一起聊天”，而是分层：

- Codex 主线程负责判断。
- Codex 子代理负责承接任务节点。
- Cursor Agent CLI 负责执行。
...
脚本会检查 `CODEX_CURSOR_CHILD_THREAD=1`，强制 Cursor Agent 委派只能发生在 Codex 子线程里。
...
主 Codex 当 leader。Codex 子代理承接任务。Cursor Agent CLI 执行具体工作。DeepSeek 消化大量上下文和重复劳动。session 复用池把上下文热起来，fingerprint 和 lease 管住并行，审计产物和验证脚本把链路兜住。
