...
def install_simulated_plugin(temp_root: Path) -> tuple[Path, Path]:
    ...
    plugin_root = codex_home / "plugins" / "cache" / "aiskyhub" / "codex-with-cursor" / "1.0.0" / "skills" / "codex-with-cursor"
    project_root.mkdir(parents=True, exist_ok=True)
    shutil.copytree(SCRIPTS, plugin_root / "scripts")
    shutil.copy2(WORKFLOW / "CODEX_WITH_CURSOR.md", plugin_root / "CODEX_WITH_CURSOR.md")
...
def test_marketplace_install_uses_project_cwd_for_default_paths() -> None:
    with tempfile.TemporaryDirectory(prefix="codex_with_cc_marketplace_paths_") as tmp:
        ...
        plugin_scripts = codex_home / "plugins" / "cache" / "aiskyhub" / "codex-with-cursor" / "1.0.0" / "skills" / "codex-with-cursor" / "scripts"
        env = {
            **os.environ,
        ...
        probe = subprocess.run(
            [
                ...
                (
                    "import json; "
                    "from codex_with_cursor_runtime.paths import repo_root, workflow_relative_path; "
        ...
        assert rel
        assert rel.endswith("/skills/codex-with-cursor") or rel.endswith("\\skills\\codex-with-cursor") or Path(rel).is_absolute()

        delegate = plugin_scripts / "delegate_to_cursor.py"
        task_root = project_root / ".codex" / "codex_with_cursor" / "tasks" / "install"
        task_root.mkdir(parents=True)
        task_file = task_root / "repo-install-dry-run.md"
        task_file.write_text(compliant_task("repo install dry run"), encoding="utf-8")
        dry_run = run_python(
            delegate,
            "-TaskFile",
            str(task_file),
            "-WorkflowId",
            "wf-repo-install-paths",
            "-TaskId",
            "task-repo-install-paths",
            "-Role",
            "researcher",
            "-SessionKey",
            "repo-install-paths",
        ...
        chain_run = run_python(
            real_chain,
            "-Name",
            "repo-install-defaults",
            "-SessionKey",
            "repo-install-session",
            cwd=project_root,
            env=env,
        )
        assert chain_run.returncode == 0, chain_run.stdout + chain_run.stderr
        validation_root = project_root / ".codex" / "codex_with_cursor" / "cursor-delegate-validation" / "repo-install-defaults"
    ...
    with tempfile.TemporaryDirectory(prefix="codex_with_cc_source_cwd_") as tmp:
        ...
        probe = subprocess.run(
            [
                sys.executable,
                "-c",
                "from codex_with_cursor_runtime.paths import repo_root; print(repo_root())",
            ],
            cwd=str(project_root),
            text=True,
            capture_output=True,
